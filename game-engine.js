...
// ── 阶段7：AGE 30 ───────────────────────────────────────────
{
  ...
  choices:[
    {tag:'A',text:'"接手全部经纪权，我要做这个家族的新太后"',
     delta:{liuliang:100,taihou:80,ziwo:-50,shenjia:150,caopan:150,shangyebantu:100},type:'名利',
...
// ── 阶段8：AGE 32 ───────────────────────────────────────────
{
  ...
  choices:[
    {tag:'A',text:'"拍！全拍！我连上厕所都可以直播！"',
     delta:{liuliang:300,taihou:100,ziwo:-60,shenjia:300,caopan:100,shangyebantu:200},type:'名利',
    ...
    {tag:'B',text:'"低俗真人秀免了，我要做高端访谈节目"',
     delta:{liuliang:50,taihou:-50,ziwo:100,shenjia:-30,caopan:-20,shangyebantu:20},type:'独立',
...
// ── 阶段9：AGE 35 ───────────────────────────────────────────
{
  ...
  choices:[
    ...
    {tag:'B',text:'"我是个坚强的笨女人，不需要联姻"',
     delta:{liuliang:40,taihou:-40,ziwo:90,shenjia:20,caopan:20},type:'独立',
...
// ── 阶段10：AGE 40 ──────────────────────────────────────────
{
  ...
  choices:[
    {tag:'A',text:'"把我的脸印在所有能卖钱的东西上！"',
     ...
     feedback:'全面扩张商业版图。<strong>美国国税局IRS上门调查税务漏洞</strong>，太后让你顶包。你拿出证据让全家按股份比例共同承担，Kris当场翻脸。',extraDelta:{taihou:-150,ziwo:100,caopan:100}},
    {tag:'B',text:'"高端化。做卡戴珊的爱马仕，不做沃尔玛"',
     delta:{liuliang:-20,taihou:-30,ziwo:70,shenjia:100,caopan:30,shangyebantu:200},type:'独立',
...
// ── 阶段11：AGE 50 ──────────────────────────────────────────
{
  ...
  choices:[
    ...
    {tag:'B',text:'"我要自由。再见妈妈。"',
     delta:{liuliang:-100,taihou:-200,ziwo:200,shenjia:-50,caopan:-200,shangyebantu:0},type:'独立',
...
var ENDINGS = [
  ...
  // ── 主线结局1：Queen never cry ──────────────────────────
  {id:'taihou2',
   condition:function(s){return s.caopan>=300&&s.taihou>=200;},
  ...
  // ── 主线结局2：老娘独享经济 ──────────────────────────────
  {id:'wallstreet',
   condition:function(s){return s.shangyebantu>=1500&&s.taihou<0;},
  ...
  // ── 主线结局4：再见了所有卡戴珊战士 ────────────────────
  {id:'retire',
   condition:function(s){return s.ziwo>=250&&s.shenjia<100;},
  ...
  // ── 主线结局5：疯狂星期四的M女王 ──────────────────────
  {id:'fastfood',
   condition:function(s){return s.shangyebantu>=2000&&s.ziwo<30;},
  ...
  // ── 主线结局6：雅思8.5高学历拥有者 ──────────────────────
  {id:'scholar',
   condition:function(s){return s.ziwo>=200&&s.liuliang<50&&s.taihou<-100;},
...
function onChoiceClick(stageIdx,choiceIdx){
  ...
  updateStatsUI();
  if(GameState.stats.ziwo<=0){
    setTimeout(function(){showEnding();},1800);
    return;
  }
  // 新增：其他维度失败检测
  if(GameState.stats.shenjia<=0||GameState.stats.liuliang<=0||GameState.stats.taihou<=0){
    setTimeout(function(){showEnding();},1800);
    return;
  }
